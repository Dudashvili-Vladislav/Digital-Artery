...
class FeedTopAPIView(ListAPIView):
    ...
    def get_queryset(self) -> QuerySet:
        ...
        return Post.objects.all().order_by('?')[:10]


class FeedTrendingAPIView(ListAPIView):
...
class FeedAPIView(ListAPIView):
    ...
    def get_queryset(self) -> QuerySet:
        ...
        followers = user.following
        follower_ids = list(followers.values_list('id', flat=True))
        print(len(Post.objects.filter(user__in=follower_ids).order_by('-timestamp')))
        return Post.objects.filter(user__in=follower_ids).order_by('-timestamp')[(page-1) * n_items:page * n_items]
